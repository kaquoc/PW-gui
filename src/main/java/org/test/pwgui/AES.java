...
public class AES {
    ...
    public AES(String login,String password)  {
        try {
            this.login = login;
    ...
    public AES(String login,String password, String iv){
        try {
            this.login  = login;
            this.password = password;
            this.key = hashKey(password);
    ...
    //method use to get Initialization vector for org.test.pwgui.AES-CBC cipher mode
    private static IvParameterSpec createIV() {
        //Using org.test.pwgui.AES with mode CBC (Cipher Block Chaining)
        //generating an IV of 16 bytes, for org.test.pwgui.AES 128 bits encryption blocks
        byte[] iv = new byte[16];
